...
/**
 * A binder for datagram-oriented listeners (UDP).
 *
 * @author Aaron Caraveo
 */
final class Bnd_UnicastListener extends Bnd_Binder
{
  //===========================================================================
  //  CONSTRUCTORS
  //===========================================================================
    ...
    throws Excp_Binder
  {
    ...
    try
    {
      ...
      boolean isUnicast =
          listenAddress.getSynapseType() == Syn_Type.UNICAST;
      
      if ( !(isUnicast && listenAddress.getSynapseMode() == Syn_Mode.LISTENER) )
      {
        throw new Excp_Binder(cellName,
                              cellInstanceID,
                              synName,
  ...
  // ListenThread
  //
  // Listens for join requests from Unicast peers.
  //---------------------------------------------------------------------------
  private final class ListenThread extends Thread
  {
    ListenThread ()
    {
      super(cellName + " (" + cellInstanceID + ") Unicast Listener (" +
            synName + ")");
        ...
        if (terminateListener.isNotSet())
        {
          ...
          catch (Excp_SynapseFatal tfe)
          {
            if (terminateListener.isNotSet())
            {
              ...
              closeSynapse(listenSynapse, getName());
              continue ListenLoop;
            }
          }
        }
          ...
          try
          {
            ...
            if (msg instanceof Msg_JoinRequest)
            {
              ...
              nucleus.getCell().logEvent(EventType.RUNTIME,
                                         getName(),
                                         "RUNTIME,BINDER_UNICAST_LISTENER,LISTEN,1,JoinReq received from " + listenSynapse.getRemoteName());
              nucleus.getCell().logEvent(EventType.RUNTIME,
                                         getName(),
                                         "RUNTIME,BINDER_UNICAST_LISTENER,LISTEN,2," + joinReqMsg.toString());
              ...
              if (joinReqMsg.synapseDomain.equals(listenSynapse.getDomain()))
              {
                ...
                {
                  // Create fromPeer synapse
                  // will get synapse type/mode/domain from sendSynapseName
                  peerSynapse = Syn_Factory.createSynapse(joinReqMsg.sendSynapseName);
                  
                  // Open fromPeer synapse
                  peerSynapse.open(null);

                  // Connect to peer
                  peerSynapse.connect(Syn_Factory.createAddress(joinReqMsg.sendSynapseName));
                  ...
                  nucleus.getCell().logEvent(EventType.RUNTIME,
                                             getName(),
                                             "RUNTIME,BINDER_UNICAST_LISTENER,LISTEN,3," + joinAckMsg.toString());
                ...
                {                  
                  // Create toPeer synapse
                  // will get synapse type/mode/domain from sendSynapseName
                  peerSynapse = Syn_Factory.createSynapse(joinReqMsg.receiveSynapseName);
                  
                  // Open toPeer synapse
                  peerSynapse.open(null);

                  // Connect to peer
                  peerSynapse.connect(Syn_Factory.createAddress(joinReqMsg.receiveSynapseName));
                  ...
                  if (peerInfo != null)
                  {
                    ...
                    nucleus.getCell().logEvent(EventType.RUNTIME,
                                               getName(),
                                               "RUNTIME,BINDER_UNICAST_LISTENER,LISTEN,4," + joinAckMsg.toString());
            ...
            else
            {
              nucleus.getCell().logEvent(EventType.ERROR,
                                         getName(),
                                         "Invalid message received from " + listenSynapse.getRemoteName());
            }  // if join request
          }
          catch (Excp_SynapseFatal tfe)
          {
  ...
  private final class PeerThread extends Thread
  {
    ...
    {
      super(cellName + " (" + cellInstanceID + ") Unicast Listener Peer (" +
