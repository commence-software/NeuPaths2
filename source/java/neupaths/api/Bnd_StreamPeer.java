...
/**
 * A binder for connection-oriented peers (TCP and Unix).
 *
 * @author Aaron Caraveo
 */
final class Bnd_StreamPeer extends Bnd_Binder
{
  //===========================================================================
  //  CONSTRUCTORS
  //===========================================================================
    ...
    throws Excp_Binder
  {
    ...
    try
    {
      ...
      boolean isStream =
          joinAddress.getSynapseType() == Syn_Type.STREAM;
      
      if ( !(isStream && joinAddress.getSynapseMode() == Syn_Mode.PEER) )
      {
        throw new Excp_Binder(cellName,
                              cellInstanceID,
                              synName,
    ...
    try
    {
      // will get synapse type/mode/domain from listenAddress
  ...
  private
  void
  performJoin (String threadName)
    throws Excp_NeuPaths
  {
    ...
    // Connect to peer
    sendSynapse.connect(joinAddress);
    ...
    sendMessage(sendSynapse, joinReqMsg, threadName);
    ...
    receiveSynapse.connect(joinAddress);
    ...
    sendMessage(receiveSynapse, joinReqMsg, threadName);
  ...
  private final class JoinThread extends Thread
  {
    JoinThread ()
    {
      super(cellName + " (" + cellInstanceID + ") Stream Join (" +
            synName + ")");
  ...
  private final class PeerThread extends Thread
  {
    PeerThread ()
    {
      super(cellName + " (" + cellInstanceID + ") Stream Peer (" +
            receiveSynapse.getInstanceID() + ")");
